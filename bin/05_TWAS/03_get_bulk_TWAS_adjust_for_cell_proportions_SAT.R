...
ukbb_cell_prop <- fread("/mnt/data/share/yinly/Cell_deconvolution/Cell_composition/ukbb_obesity_adiposeSAT_common.tsv.gz")
cellnames <- colnames(ukbb_cell_prop)[2:ncol(ukbb_cell_prop)]
# cellnames_new <- gsub("Micro/Macro","MicroMacro",cellnames)
...
for(disorder in disorders){
    ...
    for (i in 1:no_genes) {
        ...
        input <- cbind(Y,X,PCs_active,Cell_prop)
    ...
    outfile_prefix <- paste0("/home/yinly/Cell_deconvolution/05_TWAS/res/res_",tissue,"_bulk_adjusted/")
