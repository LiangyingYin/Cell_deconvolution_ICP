...
files <-list.files(path=file_dir,pattern="ukbb_SAT_")

prognosis_filepath <- "/home/yinly/Cell_deconvolution/03_Analyses/data/CAD-related-Mortality-and-hospitalization-till-20221219.csv"
...
mortality_index <- which(!is.na(prognosis$CAD_mortality))
CAD_mortality <- rep(0,nrow(prognosis))
CAD_mortality[mortality_index] <- 1

prog <- cbind(prognosis$eid,CAD_mortality)
colnames(prog) <- c("IID","CAD_mortality")
...
for(j in 1:length(files)){
    ...
    # expression_file_dir = "/home/yinly/Cell_deconvolution/02_Cell_deconvolution/res/all/ukbb_adiposeSAT_all_ASPC.csv" 
    # celltype <- gsub("/home/yinly/Cell_deconvolution/02_Cell_deconvolution/res/all/ukbb_SAT_","",expression_file_dir)
    celltype <- gsub("ukbb_SAT_","",files[j])
    ...
    outfile_prefix <- "/home/yinly/Cell_deconvolution/03_Causal_Inference_CTS/res/res_CAD_mortality_prognosis_SAT/"
