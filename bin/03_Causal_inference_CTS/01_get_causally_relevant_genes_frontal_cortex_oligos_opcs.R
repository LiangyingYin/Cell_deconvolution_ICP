...
file_dir <- "/mnt/data/share/yinly/Cell_deconvolution/CTS/all_subgroup/"
files <-list.files(path=file_dir,pattern="ukbb_cortex_")
# files <- c("ukbb_cortex_OPCs.csv.gz","ukbb_cortex_Oligos.csv.gz")

causal_mat <- NULL
tissue <-"Frontal_Cortex"
res_prefix <- "/home/yinly/Cell_deconvolution/03_Causal_Inference_CTS/res_subgroup/res_causal_genes/"
for (disorder in disorders){
    cat("The causal inference for",disorder,"begins!\n")
    IID <- pheno$FID
    outcome <- pheno[,..disorder] # colnames of variables related to depression: Depression,Psychosis(1142),Bipolar(1680),Anxiety(7495)
    ...
    for (file in files){
        ...
        celltype <- gsub("/mnt/data/share/yinly/Cell_deconvolution/CTS/all_subgroup/ukbb_cortex_","",expression_file_dir)
        ...
        outfile_prefix <- paste0("/home/yinly/Cell_deconvolution/03_Causal_Inference_CTS/res_subgroup/res_",tissue,"_",disorder,"/")
        ...
        # res_filename <- gsub(".Rdata",".csv",file)
...
fwrite(as.data.frame(causal_mat),paste0(res_prefix,tissue,"_causal_genes_oligos_opcs.csv"),sep="\t")
