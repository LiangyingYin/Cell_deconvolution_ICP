...
disorders <- c("T2DM","T1DM","Prediabetes","T2DM_insulin","T2DM_admission")
# disorders <- c("T2DM_insulin","T2DM_admission")
# disorders <- "T2DM"
file_dir <- "/mnt/data/share/yinly/Cell_deconvolution/CTS/CTS_B-mode/"
files <-list.files(path=file_dir,pattern="ukbb_pancreas_")
...
for (disorder in disorders){
    ...
    for (file in files){
        ...
        celltype <- gsub("ukbb_pancreas_","",file)
        celltype <- gsub(".csv.gz","",celltype)
        expr = fread(expression_file_dir )

        data <- merge(expr,outcome_all,by="IID", sort=FALSE) # merge expression with outcome variable
        ...
        outfile_prefix <- paste0("/home/yinly/Cell_deconvolution/03_Causal_Inference_CTS/res/res_",tissue,"_",disorder,"/")
        ...
        # res_filename <- gsub(".Rdata",".csv",filepath)
