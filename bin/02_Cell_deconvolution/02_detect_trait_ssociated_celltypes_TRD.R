...
TRD <- fread("/exeh_3/yinly/Cell_deconvolution/02_Cell_deconvolution/data/Anti_depressant_treatment_resistant_result.csv")
colnames(TRD) <- c("IID","TRD")
Dep_TRD <- merge(Dep_all,TRD,by="IID",all.Y=TRUE,sort=FALSE)
Dep_index <- which(Dep_TRD$Depression==1)
Dep_TRD_selected <- Dep_TRD[Dep_index,]

ukbb_cell_common <- fread("/exeh_3/yinly/Cell_deconvolution/02_Cell_deconvolution/res/UKBB/ukbb_depression_cortex_common.tsv")
cell_common_dat <- merge(ukbb_cell_common,Dep_TRD_selected,by="IID",sort=FALSE)

ukbb_cell_all <- fread("/exeh_3/yinly/Cell_deconvolution/02_Cell_deconvolution/res/UKBB/ukbb_depression_cortex_all.tsv")
cell_all_dat <- merge(ukbb_cell_all,Dep_TRD_selected,by="IID",sort=FALSE)
...
cell_ignorant_dat <- merge(ukbb_cell_ignorant,Dep_TRD_selected,by="IID",sort=FALSE)

feat_len <- ncol(cell_all_dat)-ncol(Dep_TRD_selected) + 1
...
for(i in 2:feat_len){
    ...
    common_obj <- biglm(as.numeric(cell_common_dat$TRD) ~ common_cell_prop,data=cell_common_dat)
    ...
    all_obj <- biglm(as.numeric(cell_all_dat$TRD) ~ all_cell_prop,data=cell_all_dat)
    all_res <- summary(all_obj)$mat[2,c(1,5)]
    # ignorant
    ignorant_cell_prop <- unlist(subset(cell_ignorant_dat,select=i))
    ignorant_obj <- biglm(as.numeric(cell_ignorant_dat$TRD) ~ ignorant_cell_prop,data=cell_ignorant_dat)
...
fwrite(as.data.frame(res_adjust),"/exeh_3/yinly/Cell_deconvolution/02_Cell_deconvolution/res/res_disease_status_assoc/ukbb_depression_frontal_cortex_TRD_assoc.txt",sep="\t")
