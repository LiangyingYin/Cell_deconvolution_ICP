...
for (disorder in disorders){
    ...
    IID <- pheno$FID
    target <- pheno[,..disorder] # colnames of variables related to depression: Depression,Psychosis(1142),Bipolar(1680),Anxiety(7495)
    na_index <- which(is.na(target))
    target[na_index] <- 0

    outcome_all <- cbind(IID,target)
    colnames(outcome_all) <- c("IID","outcome")

    for(j in 1:length(files)){
        # get expression file path
        expression_file_dir <- paste0(file_dir,files[j])
        celltype <- gsub("/mnt/data/share/yinly/Cell_deconvolution/CTS/CTS_B-mode/ukbb_cortex_","",expression_file_dir)
