...
outcome <- rep(0,length(outcome_all))
outcome[case_index] <- 1
envir <- as.vector(unlist(meta$race))
...
label_fir <- outcome[envir_one_index]
...
label_sec <- outcome[envir_two_index]

label_binary <- outcome
...
cat("There are",sum(label_fir_binary),"T2D patients in the 1st dataset. \n")
...
cat("There are",sum(label_sec_binary),"T2D patients in the 2nd dataset. \n")
...
# employ IGCM to identify the invariant markers
...
outputfile <- "/mnt/home/yinly/projects/Cell_deconvolution/res/06_Benchmark/01_Cellmarker_identification/T2D/Diabetes_markers.RData"
save(dtrain_fir_rank,dtrain_sec_rank,merged_rank,IGCM_mat,causal_set_index,file=outputfile)
