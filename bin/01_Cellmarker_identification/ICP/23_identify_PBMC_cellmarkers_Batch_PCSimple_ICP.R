...
# Ranked genes by zMin - only genes present in both datasets
merge_and_rank_common_genes <- function(df1, df2) {
  # Ensure required columns exist
  required_cols <- c("genes", "assoc", "zMin")
  stopifnot(all(required_cols %in% colnames(df1)),
            all(required_cols %in% colnames(df2)))
  
  # Inner join by gene (only keeps genes present in both datasets)
  merged <- merge(df1, df2, by = "genes", all = FALSE, suffixes = c(".1", ".2"))
  
  # Compute average zMin
  merged$zMin <- rowMeans(cbind(as.numeric(merged$zMin.1), as.numeric(merged$zMin.2)), na.rm = FALSE)
  ...
  # Sort by zMin in descending order
  final_df <- final_df[order(-final_df$zMin), ]
...
for(i in 1:length(celltypes)){
    ...
    res_causal_merged <- merge_and_rank_common_genes(res_causal,res_causal_sec)
    ...
    outputfile <- paste0("/mnt/home/yinly/projects/Cell_deconvolution/res/06_Benchmark/01_Cellmarker_identification/PBMC/6810KABC_Batch_PCSimple_Common_ICP/6810KABC_cellmarkers_",celltype,".RData")
