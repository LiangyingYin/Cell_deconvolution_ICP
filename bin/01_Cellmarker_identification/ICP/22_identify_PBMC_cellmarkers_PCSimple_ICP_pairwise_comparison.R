...
# # Convert labels to numeric(0,1,2,3,...,n_class-1)
# label_map <- setNames(0:(length(unique(celltypes_all))-1),unique(celltypes_all))
# numeric_labels <- label_map[celltypes_all]

# data_fir <- scRNA
# data_fir <- as.matrix(data_fir)
# class(data_fir) <- "numeric"
# xgb_multi <- xgboost(data=data_fir,label=numeric_labels,objective="multi:softprob",num_class=length(celltypes),max.depth = 2, eta = 1, nthread = 10,nrounds=10,verbose=0)
# xgb_multi_rank <- xgb.importance(model = xgb_multi)
...
sample_num <- length(envir_binary)

# Conduct pairwise comparison to identify cell markers
for(i in 1:length(celltypes)){
    if(i < (length(celltypes)-1)){
       base_celltype <- celltypes[i]
       base_celltype_index <- which(celltypes_all==base_celltype)
       scRNA_base_celltype <- scRNA[base_celltype_index,]
       base_celltype_label <- rep(1,length(base_celltype_index))
       for(j in (i+1):length(celltypes)){
            paired_celltype <- celltypes[j]
            paired_celltype_index <- which(celltypes_all==paired_celltype)
            scRNA_paired_celltype <- scRNA[paired_celltype_index,]
            paired_celltype_label <- rep(0,length(paired_celltype_index))
            selected_celltype_index <- c(base_celltype_index,paired_celltype_index)
            data_fir <- rbind(scRNA_base_celltype,scRNA_paired_celltype)
            data_fir <- as.matrix(data_fir)
            class(data_fir) <- "numeric"
            label_fir_binary <- c(base_celltype_label,paired_celltype_label)
            ...
            X_assoc <- subset(X_assoc,select=causal_genes)
            ...
            for(j in initial_assoc_len:1){
                X<-envir_binary[selected_celltype_index]
            ...
            celltype_pairs <- paste0(base_celltype,"_vs_",paired_celltype)
            outputfile <- paste0("/mnt/home/yinly/projects/Cell_deconvolution/res/06_Benchmark/01_Cellmarker_identification/PBMC/6810KABC_PCSimple_ICP_pairwise/6810KABC_cellmarkers_",celltype_pairs,".RData")
            save(celltype_pairs,dtrain_fir_rank,merged_rank,IGCM_mat,causal_set_index,file=outputfile)
