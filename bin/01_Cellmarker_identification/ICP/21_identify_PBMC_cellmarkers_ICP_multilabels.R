...
Y_numeric <- numeric_labels
assoc_feats <- as.vector(unlist(xgb_multi_rank$Feature))
...
for(j in initial_assoc_len:1){
    ...
    gcm_obj <- gcm_test(X=X,Y=Y_numeric,Z=Z,regr.par=list(max_depth=4,max_delta_step=6,objective="multi:softprob"), regr.method = "xgboost")
...
outputfile <- "/mnt/home/yinly/projects/Cell_deconvolution/res/06_Benchmark/01_Cellmarker_identification/PBMC/6810KABC_Multilabels_ICP/6810KABC_cellmarkers_mulitlabels.RData"
save(celltypes,xgb_multi_rank,IGCM_mat,causal_set_index,file=outputfile)
