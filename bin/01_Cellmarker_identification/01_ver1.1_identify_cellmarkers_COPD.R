...
cell_info <- fread("/home/yinly/Cell_deconvolution/01_Cellmarker_identification/data/COPD_cellinfo_Mar13.csv")

# The following is only for test
colnames(cell_info)
outcome <- cell_info$Disease_Identity
celltype <- cell_info$Manuscript_Identity
unique(celltype)
...
external_ids <- c("001C","002C","003C","222C","034C","065C","081C","084C","092C","098C","133C","1372C","137C","052CO","056CO","137CO") # The first 13 are ctrl ids, the remaining are case ids
...
scRNA_selected <- scRNA_all[selected_index,] # 
rm(scRNA_all)

ctrl_index <- which(scRNA_selected$outcome=="Control")
ctrl <- scRNA_selected[ctrl_index,]
# unique(ctrl[,"celltype"])
# unique(ctrl[,"donor_ids"]) # overall 28 ctrls, 20 remained for training
...
# #for(i in 9:9){
    ...
    outputfile <- paste0("/home/yinly/Cell_deconvolution/01_Cellmarker_identification/res/res_COPD_Minor/COPD_cellmarkers_",celltype,".RData")
