...
normal_origi <- fread("/exeh_3/yinly/Cell_deconvolution/01_Cellmarker_identification/data/BRCA_normal.csv")
# scRNA_genes <- colnames(normal_origi)
# scRNA_genes_all <- cbind(seq(1,length(scRNA_genes)),scRNA_genes)
# colnames(scRNA_genes_all) <- c("ID","genes")
# fwrite(as.data.frame(scRNA_genes_all),"/exeh_3/yinly/Cell_deconvolution/02_Cell_deconvolution/res/res_gene_symbol_scRNA-seq/BRCA_normal_genes.csv",sep="\t")

UKBB <- fread("/exeh_3/yinly/Cell_deconvolution/01_Cellmarker_identification/SPred_genes/UKBB_Breast_Mammary_BRCA_final_SPred_genes.csv",header=TRUE)
UKBB_genes <- as.vector(unlist(UKBB[,2]))
common_genes <- intersect(colnames(normal_origi),UKBB_genes)
normal <- subset(normal_origi,select=common_genes)
#normal <- normal_origi[,common_genes]
...
for(i in 1:length(celltypes)){
    ...
    dtrain_fir <- xgb.DMatrix(data = case, label = label_fir_binary)
    ...
    outputfile <- paste0("/exeh_3/yinly/Cell_deconvolution/01_Cellmarker_identification/res/res_Breast_normal_SPred_final/BRCA_cellmarkers_",celltype,".RData")
